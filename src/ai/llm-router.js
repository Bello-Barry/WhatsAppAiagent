...
const getGeminiResponse = async (prompt, history, systemPrompt) => {
  if (!gemini) {
    throw new Error('Gemini API key not configured.');
  }
  
  const model = 'gemini-2.5-flash';
  logger.info(`Querying Gemini model: ${model}`);
...
const getOpenAIResponse = async (prompt, history, systemPrompt) => {
  // Placeholder for OpenAI integration
  logger.warn('OpenAI provider is not yet implemented.');
  return `(Mock OpenAI Response) You said: "${prompt}"`;
};

const getDeepSeekResponse = async (prompt, history, systemPrompt) => {
...
export const getLLMResponse = async (prompt, history, agentConfig) => {
  ...
  return provider(prompt, history, agentConfig.systemPrompt);
};
